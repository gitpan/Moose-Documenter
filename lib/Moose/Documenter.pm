...
sub inherited_attributes {
  ...
  foreach my $parent ( @{ $self->{parents} } ) {
    ...
    if( $parent eq 'Moose::Object' ) {
      $parent_doc = Moose::Documenter->new( $self->{moose_lib}, $parent );
    }
    else { $parent_doc = Moose::Documenter->new( $self->{user_lib}, $parent ); }
...
sub roles {
  ...
  foreach my $role ( @{ $meta_object->roles } ) {
    ...
    my $role_doc = Moose::Documenter->new( $self->{user_lib}, $rolename );
...
Moose::Documenter - class for getting Moose documentation for your Moose classes

=head1 SYNOPSIS

Using the library:

  use Moose::Documenter;
  my $doc = Moose::Documenter->new('/path/to/Moose/classes/','module::name::of::class');
...
Returns a Moose::Documenter for the specified moose class.  Takes a path to add to the perl lib path for the system to find your moose class and the name of the moose class.
